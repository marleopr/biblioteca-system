...
export const BookCountTooltip = ({ authorId, bookCount }: BookCountTooltipProps) => {
  const [isOpen, setIsOpen] = useState(false);
  
  const { data: topBooks, isLoading } = useQuery({
    queryKey: ['top-books-author', authorId],
    queryFn: () => loanService.getTopBooksByAuthor(authorId, 5),
